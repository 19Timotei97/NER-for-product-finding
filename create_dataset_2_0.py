...
def load_dataset(json_path: str, tokenizer: Any, max_len: int, split_percent: float = 0.8) -> Tuple[CrawlingDataset, CrawlingDataset, float]:
    """Loads a JSON dataset and returns a split version (training/validation)
    ...
    logging.info(f'Neg/Pos Ratio: {neg_pos_ratio}')
    logging.info(f'Train Samples: {len(train_inputs)} | Val Samples: {len(val_inputs)}')

    return CrawlingDataset((train_inputs, train_targets), tokenizer, max_len), \
           CrawlingDataset((val_inputs, val_targets), tokenizer, max_len), \
           neg_pos_ratio
